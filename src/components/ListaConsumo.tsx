...
export default function ListaConsumo({ value, onChange, placeholderPessoa = 'Pessoa' }: Props) {
  ...
  return (
    <div className="space-y-2">
      {value.map((it, i) => (
        <div key={i} className="rounded-xl bg-gray-50 p-2.5 space-y-1.5">
          <div className="flex gap-1.5">
            <input
              type="text"
              placeholder={placeholderPessoa}
              className="flex-1 min-w-0 rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm outline-none focus:border-gray-400"
              value={it.pessoa}
          ...
          <div className="flex gap-1.5 items-center">
            <div className="flex items-center flex-1 rounded-lg border border-gray-200 bg-white">
      ...
      <div className="flex items-center justify-between">
        <button type="button" onClick={adicionar} className="text-sm font-medium text-gray-900 hover:underline">
          + Adicionar
        </button>
        {value.length > 0 && <span className="text-sm text-gray-400">Total: {formatarMoeda(total)}</span>}
      </div>
    </div>
  )
}
